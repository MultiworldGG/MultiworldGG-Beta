...
from WebHostLib.models import (db, commit, flush, Generation, Lobby, LobbyApworld, LobbyAuxiliaryApworld,
                               LobbyMetaYaml, LobbyPlayer, LobbyYaml,
                               LOBBY_CLOSED, LOBBY_DONE, LOBBY_GENERATING, LOBBY_LOCKED, LOBBY_OPEN)
...
class TestLobbyFiles(TestBase):
    ...
    def setUp(self):
        ...
        with self.app.app_context():
            ...
            flush()
            host = LobbyPlayer(lobby_id=lobby.id, session_id=self.host, player_name="Host", is_ready=True)
            member = LobbyPlayer(lobby_id=lobby.id, session_id=self.member, player_name="Member", is_ready=True)
            flush()
            self.original = f"name: HostSlot\ngame: {GAME}\n{GAME}:\n  progression_balancing: 12\n".encode()
            first = LobbyYaml(lobby_id=lobby.id, player_id=host.id, filename="host.yaml",
                              yaml_player_name="HostSlot", yaml_game=GAME, content=self.original)
            second = LobbyYaml(lobby_id=lobby.id, player_id=member.id, filename="member.yaml",
                               yaml_player_name="MemberSlot", yaml_game=GAME,
                               content=self.original.replace(b"HostSlot", b"MemberSlot"))
            flush()
    ...
    def _set_lobby(self, **values):
        with self.app.app_context():
            lobby = Lobby.get(id=self.lobby_id)
            for key, value in values.items():
                setattr(lobby, key, value)
            commit()

    def _auxiliary_count(self):
        with self.app.app_context():
            return db.session.scalar(
                select(func.count()).select_from(LobbyAuxiliaryApworld)
                .where(LobbyAuxiliaryApworld.lobby_id == self.lobby_id)
            )

    def _auxiliary_path(self, auxiliary_id):
        with self.app.app_context():
            return LobbyAuxiliaryApworld.get(id=auxiliary_id).storage_path
    ...
    def test_meta_permissions_and_generation_states(self):
        ...
        self._set_lobby(state=LOBBY_LOCKED)
    ...
    def test_server_generation_applies_meta_and_preserves_player_uploads(self):
        ...
        with self.app.app_context():
            generation = Generation.get(id=Lobby.get(id=self.lobby_id).generation_id)
            options = restricted_loads(generation.options)
            self.assertEqual(len(options), 2)
            self.assertEqual({slot["progression_balancing"].value for slot in options.values()}, {73})
            self.assertEqual(LobbyYaml.get(id=self.host_yaml_id).content, self.original)

    def test_invalid_meta_application_does_not_queue_generation(self):
        self.upload_meta(META.replace(b"73", b"not_a_valid_value"))
        with self.app.app_context():
            generations_before = db.session.scalar(select(func.count()).select_from(Generation))
        response = self.client.post(self.url + "/generate")
        self.assertEqual(response.status_code, 400, response.get_json())
        with self.app.app_context():
            lobby = Lobby.get(id=self.lobby_id)
            self.assertEqual(lobby.state, LOBBY_OPEN)
            self.assertIsNone(lobby.generation_id)
            self.assertEqual(db.session.scalar(select(func.count()).select_from(Generation)), generations_before)
    ...
    def test_auxiliary_limit_is_shared_per_world_not_per_yaml(self):
        ...
        with self.app.app_context():
            LobbyApworld(lobby_id=self.lobby_id, yaml_id=self.host_yaml_id, game_name=GAME,
    ...
    def test_auxiliary_limit_is_independent_for_other_worlds(self):
        ...
        with self.app.app_context():
            host_player_id = LobbyYaml.get(id=self.host_yaml_id).player_id
            other_world = LobbyYaml(lobby_id=self.lobby_id, player_id=host_player_id, filename="custom.yaml",
        ...
        with patch.dict(self.app.config, {"LOBBY_AUXILIARY_APWORLD_GAMES": ["UZDoom"]}):
            for game, expected in (("UZDoom - Doom II", 201), ("My UZDoom World", 400), ("uzdoom - Doom II", 400)):
                with self.subTest(game=game):
                    with self.app.app_context():
                        LobbyYaml.get(id=self.host_yaml_id).yaml_game = game
                        commit()
    ...
    def test_auxiliary_permissions_allowlist_and_validation(self):
        ...
        self._set_lobby(allow_custom_apworlds=False)
        self.assertEqual(self.upload_auxiliary().status_code, 400)
        self._set_lobby(allow_custom_apworlds=True)
        ...
        downloaded.close()
        self._set_lobby(state=LOBBY_GENERATING)
        self.assertEqual(self.upload_auxiliary("another.apworld").status_code, 400)
    ...
    def test_auxiliary_filename_conflicts_and_removal(self):
        ...
        path = self._auxiliary_path(auxiliary_id)
        self.assertEqual(self.client.delete(file_url).status_code, 200)
    ...
    def test_file_access_is_scoped_to_lobby(self):
        ...
        with self.app.app_context():
            ...
            flush()
            LobbyPlayer(lobby_id=another.id, session_id=self.host, player_name="Host")
    ...
    def test_stale_lobby_cleanup_removes_meta_and_auxiliary_files(self):
        ...
        response = self.upload_auxiliary()
        path = self._auxiliary_path(response.get_json()["id"])
        self._set_lobby(state=LOBBY_CLOSED, last_activity=utcnow() - timedelta(hours=2))
        with self.app.app_context():
            engine = db.engine
        previous = autolauncher._engine
        autolauncher._engine = engine
        try:
            autolauncher.cleanup({"ROOM_AUTO_DELETE": 0})
        finally:
        ...
        with self.app.app_context():
            ...
            self.assertEqual(db.session.scalar(
                select(func.count()).select_from(LobbyMetaYaml).where(LobbyMetaYaml.lobby_id == self.lobby_id)
            ), 0)
        self.assertEqual(self._auxiliary_count(), 0)

    def test_participant_page_contains_compact_meta_control_only(self):
        response = self.other.get(f"/play/lobby/{to_url(self.lobby_id)}")
        self.assertEqual(response.status_code, 200)
