...
foreach ($World in $WorldsWithPyProject) {
    ...
    $eggname = "worlds." + "$World" + ".egg-info"
    $EggPath = Join-Path $SrcDir $eggname
    ...
    try {
        ...
        # Run build command
        Write-Host "  Installing editable module..." -ForegroundColor Cyan
        $BuildResult = & python -m pip install -e . 2>&1
