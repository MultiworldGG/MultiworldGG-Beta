...
begin
  ...
  begin
    ...
    // Run ensurepip to ensure pip is available (will skip if already installed)
    if Exec('python', '-m ensurepip', '', SW_HIDE, ewWaitUntilTerminated, I) then
    begin
      ...
      Log('Failed to ensure pip via python command');
  end;
  
  // Try Windows Python Launcher
  if Exec('py', '--version', '', SW_HIDE, ewWaitUntilTerminated, I) then
  begin
    Log('Python found via Windows Python Launcher (py)');
    // Run ensurepip to ensure pip is available (will skip if already installed)
    if Exec('py', '-m ensurepip', '', SW_HIDE, ewWaitUntilTerminated, I) then
    begin
      ...
      Log('Failed to ensure pip via py command');
  end;
  
  // Check HKEY_LOCAL_MACHINE first
  if RegGetSubkeyNames(HKEY_LOCAL_MACHINE, 'SOFTWARE\Python\PythonCore', PythonVersions) then
  begin
    Log('Found ' + IntToStr(GetArrayLength(PythonVersions)) + ' Python versions in HKLM');
    for I := 0 to GetArrayLength(PythonVersions) - 1 do
    begin
      VersionStr := PythonVersions[I];
      Log('Checking Python version: ' + VersionStr);
      if (Length(VersionStr) >= 3) and (Copy(VersionStr, 1, 1) >= '0') and (Copy(VersionStr, 1, 1) <= '9') and (Copy(VersionStr, 2, 1) = '.') and (Copy(VersionStr, 3, 1) >= '0') and (Copy(VersionStr, 3, 1) <= '9') then
      begin
        MajorVersion := StrToIntDef(Copy(VersionStr, 1, 1), 0);
        MinorVersion := StrToIntDef(Copy(VersionStr, 3, 1), 0);
        Log('Parsed version: Major=' + IntToStr(MajorVersion) + ', Minor=' + IntToStr(MinorVersion));
        if (MajorVersion > 3) or ((MajorVersion = 3) and (MinorVersion >= 8)) then
        begin
          // Try to find the actual python.exe
          if RegQueryStringValue(HKEY_LOCAL_MACHINE, 'SOFTWARE\Python\PythonCore\' + VersionStr + '\InstallPath', '', PythonPath) then
          begin
            if FileExists(PythonPath + '\python.exe') then
            begin
              Log('Found suitable Python version in HKLM: ' + VersionStr + ' at ' + PythonPath);
              Result := True;
              Exit;
            end
            else
              Log('Python registry entry found but python.exe not found at: ' + PythonPath);
          end;
        end
        else
          Log('Python version ' + VersionStr + ' is below minimum requirement (3.8)');
      end;
    end;
  end
  else
    Log('No Python versions found in HKLM registry');
  
  // Check HKEY_CURRENT_USER if not found in HKLM
  if not Result and RegGetSubkeyNames(HKEY_CURRENT_USER, 'SOFTWARE\Python\PythonCore', PythonVersions) then
  begin
    Log('Found ' + IntToStr(GetArrayLength(PythonVersions)) + ' Python versions in HKCU');
    for I := 0 to GetArrayLength(PythonVersions) - 1 do
    begin
      VersionStr := PythonVersions[I];
      Log('Checking Python version: ' + VersionStr);
      ...
      begin
        ...
        begin
          // Try to find the actual python.exe
          if RegQueryStringValue(HKEY_CURRENT_USER, 'SOFTWARE\Python\PythonCore\' + VersionStr + '\InstallPath', '', PythonPath) then
          begin
            if FileExists(PythonPath + '\python.exe') then
            begin
              Log('Found suitable Python version in HKCU: ' + VersionStr + ' at ' + PythonPath);
              Result := True;
              Exit;
            end
            else
    ...
    Log('No Python versions found in HKCU registry');
  
  if not Result then
    Log('No suitable Python version (3.8+) found in registry or PATH');
end;
