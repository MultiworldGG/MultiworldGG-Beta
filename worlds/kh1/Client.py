...
def launch(server_address: str = None, slot_name: str = None, password: str = None, ready_callback=None, error_callback=None):
    ...
    """
    import logging
    logging.getLogger("KH1Client")

    async def main():
        ctx = KH1Context(server_address, slot_name, password, ready_callback, error_callback)
    ...
    try:
        ...
        task = asyncio.create_task(main(), name="KH1Main")
        return task
    except RuntimeError:
        logger.critical("This is not a standalone client. Please run the MultiWorld GUI to start the KH1 client.")
        if error_callback:
            error_callback()
