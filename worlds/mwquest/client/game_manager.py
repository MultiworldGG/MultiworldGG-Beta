...
if TYPE_CHECKING:
    from .mw_quest_client import MWQuestContext


class MWQuestManager(GameManager):
    base_title = "MWQuest for AP version"
    ctx: MWQuestContext
    ...
    def build(self) -> Layout:
        ...
        self.game_view = MWQuestGameView(self.ctx.input_and_rerender)

        self.game_view_tab = self.add_client_tab("MWQuest", self.game_view)

        controls = MWQuestControlsView()

        self.add_client_tab("Controls", controls)

        game_container = self.game_view.ids["game_container"]
        self.lower_game_grid = MWQuestGrid()
        self.upper_game_grid = MWQuestGrid()
