...
from .game_manager import MWQuestManager
...
class MWQuestClientCommandProcessor(ClientCommandProcessor):
    ctx: "MWQuestContext"
...
class MWQuestContext(CommonContext):
    game = "MWQuest"
    ...
    ui: MWQuestManager
    command_processor = MWQuestClientCommandProcessor
    ...
    async def mwquest_loop(self) -> None:
        while not self.exit_event.is_set():
            ...
            if not self.mw_quest_game or not self.mw_quest_game.gameboard or not self.mw_quest_game.gameboard.ready:
                await asyncio.sleep(0.1)
                continue
            ...
            try:
                ...
                for item in new_items:
                    ...
                    self.mw_quest_game.receive_item(item.item, item.location, item.player)
                    rerender = True

                for new_remotely_cleared_location in self.checked_locations - self.locations_checked:
                    self.mw_quest_game.force_clear_location(new_remotely_cleared_location)
                    rerender = True

                if rerender:
                    self.render()

                if self.mw_quest_game.player.has_won and not self.finished_game:
    ...
    def on_package(self, cmd: str, args: dict[str, Any]) -> None:
        ...
        if cmd == "Connected":
            ...
            self.mw_quest_game = Game(self.hard_mode, self.hammer, self.extra_starting_chest)
        ...
        if cmd == "LocationInfo":
            ...
            assert self.mw_quest_game is not None
            self.mw_quest_game.gameboard.fill_remote_location_content(remote_item_graphic_overrides)
            self.render()
    ...
    def render(self) -> None:
        if self.mw_quest_game is None:
            raise RuntimeError("Tried to render before self.mw_quest_game was initialized.")

        self.ui.render(self.mw_quest_game, self.player_sprite, self.hard_mode)
    ...
    def handle_game_events(self) -> None:
        ...
        while self.mw_quest_game.queued_events:
            event = self.mw_quest_game.queued_events.pop(0)
            ...
            if isinstance(event, ConfettiFired):
                gameboard_x, gameboard_y = self.mw_quest_game.gameboard.size
    ...
    def input_and_rerender(self, input_key: Input) -> None:
        if self.mw_quest_game is None:
            return
        if not self.mw_quest_game.gameboard.ready:
            return
        self.mw_quest_game.input(input_key)
        self.render()
    ...
    def queue_auto_move(self, target_x: int, target_y: int) -> None:
        ...
        if not self.ui.game_view.focused > 1:  # Must already be in focus
            return
        self.mw_quest_game.queue_auto_move(target_x, target_y)
    ...
    def do_auto_move_and_rerender(self) -> None:
        ...
        if not self.mw_quest_game.gameboard.ready:
            return
        changed = self.mw_quest_game.do_auto_move()
        if changed:
            self.render()
    ...
    def confetti_and_rerender(self) -> None:
        ...
        if self.mw_quest_game.attempt_fire_confetti_cannon():
            self.render()
    ...
    def external_math_trap_input(self, raw: str) -> bool:
        ...
        if not self.mw_quest_game.gameboard.ready:
            return False
        if not self.mw_quest_game.active_math_problem:
            return False
        ...
        self.mw_quest_game.math_problem_replace([int(digit) for digit in raw])
    ...
    def load_kv(self) -> None:
        ...
        data = pkgutil.get_data(__name__, "mw_quest_client.kv")
        if data is None:
            raise RuntimeError("mw_quest_client.kv could not be loaded.")
...
async def main(args: Namespace) -> None:
    if not gui_enabled:
        raise RuntimeError("MWQuest cannot be played without gui.")
    ...
    ctx = MWQuestContext(args.connect, args.password, delay_intro_song=delay_intro_song)
    ctx.auth = args.name
    ...
    ctx.client_loop = asyncio.create_task(ctx.mwquest_loop(), name="Client Loop")
...
def launch(*args: str) -> None:
    from .launch import launch_mw_quest_client

    launch_mw_quest_client(*args)
