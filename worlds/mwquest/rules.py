...
def set_all_rules(world: MWQuestWorld) -> None:
...
def set_all_entrance_rules(world: MWQuestWorld) -> None:
    ...
    # MWQuest opts to create its Rule objects locally, but just to show what this would look like,
...
def set_all_location_rules(world: MWQuestWorld) -> None:
...
def set_completion_condition(world: MWQuestWorld) -> None:
...
# If your world exclusively uses Rule Builder rules (like MWQuest), it's worth trying CachedRuleBuilderWorld.
...
#   class MWQuestWorld(CachedRuleBuilderWorld): ...
...
# If you're wondering, author has checked: MWQuest is too simple to see any benefits, so we'll stick with "World".
