<<<<<<< HEAD:worlds/ladx/LinksAwakeningClient.py
import Utils
...
class LinksAwakeningContext(CommonContext):
    ...
    def __init__(self, server_address: str | None, password: str | None, magpie: bool, ready_callback=None, error_callback=None) -> None:
...
def launch(server_address: str = None, password: str = None, ready_callback=None, error_callback=None,
           magpie: bool = True, diff_file: str = None):
    ...
    async def main():
        ...
        ctx = LinksAwakeningContext(server_address, password, magpie, ready_callback, error_callback)
        ...
        ctx.ui.base_title = apname + " | Link's Awakening"
        ctx.server_task = asyncio.create_task(server_loop(ctx), name="server loop")
        await ctx.server_auth()

        ctx.la_task = create_task_log_exception(ctx.run_game_loop())

        # Start the game if ROM was patched
        if rom_file:
            run_game(rom_file)
    ...
    import colorama
    Utils.init_logging("LinksAwakeningClient", exception_logger="Client")
    ...
    try:
        ...
        task = asyncio.create_task(main(), name="LinksAwakeningMain")
        return task
    except RuntimeError:
        logger.critical("This is not a standalone client. Please run the MultiWorld GUI to start the Link's Awakening client.")
        if error_callback:
            error_callback()
...
def main(server_address: str = None, password: str = None, ready_callback=None, error_callback=None, 
    ...
    launch(server_address, password, ready_callback, error_callback, magpie, diff_file)
=======
import ModuleUpdate
ModuleUpdate.update()
...
def launch(*launch_args):
    ...
    asyncio.run(main())
    colorama.deinit()
>>>>>>> main:worlds/ladx_beta/LinksAwakeningClient.py
