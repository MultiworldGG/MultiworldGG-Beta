...
def link_entrances(multiworld, player):    
    ...
    if multiworld.worlds[player].options.entrance_shuffle == 'vanilla':
        ...
        if not multiworld.worlds[player].shuffle_ganon:
...
def link_inverted_entrances(multiworld, player):
...
def connect_simple(multiworld, exitname, regionname, player):
    multiworld.get_entrance(exitname, player).connect(multiworld.get_region(regionname, player))


def connect_entrance(multiworld, entrancename: str, exitname: str, player: int):
...
def connect_exit(multiworld, exitname, entrancename, player):
    entrance = multiworld.get_entrance(entrancename, player)
    exit = multiworld.get_entrance(exitname, player)
...
def connect_two_way(multiworld, entrancename, exitname, player):
    entrance = multiworld.get_entrance(entrancename, player)
    exit = multiworld.get_entrance(exitname, player)
...
def scramble_holes(multiworld, player):
...
def scramble_inverted_holes(multiworld, player):
    hole_entrances = [('Kakariko Well Cave', 'Kakariko Well Drop'),
                      ('Bat Cave Cave', 'Bat Cave Drop'),
                      ('North Fairy Cave', 'North Fairy Cave Drop'),
                      ('Lost Woods Hideout Stump', 'Lost Woods Hideout Drop'),
...
def connect_random(multiworld, exitlist, targetlist, player, two_way=False):
...
def connect_mandatory_exits(multiworld, entrances, caves, must_be_exits, player):
...
def connect_caves(multiworld, lw_entrances, dw_entrances, caves, player):
    """This works inplace"""
    multiworld.random.shuffle(lw_entrances)
    multiworld.random.shuffle(dw_entrances)
    multiworld.random.shuffle(caves)
...
def connect_doors(multiworld, doors, targets, player):
    """This works inplace"""
    multiworld.random.shuffle(doors)
...
def skull_woods_shuffle(multiworld, player):
...
def simple_shuffle_dungeons(multiworld, player):
    skull_woods_shuffle(multiworld, player)
    ...
    if multiworld.worlds[player].options.mode == 'open' or (multiworld.worlds[player].options.mode == 'inverted' and multiworld.worlds[player].shuffle_ganon):
    ...
    if multiworld.worlds[player].options.mode not in ['open', 'inverted'] or (multiworld.worlds[player].options.mode == 'inverted' and not multiworld.worlds[player].shuffle_ganon):
...
def crossed_shuffle_dungeons(multiworld, player: int):
...
def inverted_crossed_shuffle_dungeons(multiworld, player: int):
...
def unbias_some_entrances(multiworld, Dungeon_Exits, Cave_Exits, Old_Man_House, Cave_Three_Exits):
...
def plando_connect(multiworld, player: int):
