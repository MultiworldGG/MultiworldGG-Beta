...
* `sorting_priorities` : Determines how various categories of checks should be grouped together and sorted in the tracker tab. Valid category names are: `default`, `hinted`, `excluded`, `glitched`, `hinted_glitched`, `excluded_glitched`, `disconnected`, and `other`. Each category is given a non-negative number representing its priority, with lower numbers being sorted higher on the list. Categories with the same priority will be mixed together.
* `sorting_method` : Determines how individual locations are sorted within each priority group. Valid sorting methods are `location` (sort by location name), `region` (sort by region name, even if not visible), `label` (sort by the label currently being displayed), and `apworld` (custom sort defined by the apworld). If `apworld` is selected but the world doesn't define a custom sorting method, the sorting method falls back to `label`. 
