...
from rule_builder.rules import Has, HasAll, Rule, True_, CanReachRegion, CanReachEntrance
...
class WaffleRules:
    ...
    def set_smw_rules(self) -> None:
        ...
        for entrance_name, rule in self.connection_rules.items():
            ...
            if self.using_ut and world.multiworld.enforce_deferred_connections in ("on", "default"):
                try:
                    glitched_entrance = multiworld.get_entrance(f"{entrance_name} (Glitched)", self.player)
                except KeyError:
                    continue
                add_rule(glitched_entrance, lambda state: state.has(ItemName.glitched, self.player))
