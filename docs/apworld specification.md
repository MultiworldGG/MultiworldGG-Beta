...
* `minimum_ap_version` and `maximum_ap_version` - which if present will each be compared against the current
  MultiworldGG version respectively to filter those files from being loaded.
...
In the MultiworldGG Launcher, there is a "Build apworlds" component that will package all world folders to `.apworld`,
and add `archipelago.json` manifest files to them.  
These .apworld files will be output to `build/apworlds` (relative to the MultiworldGG root directory).  
