...
If you just want to play and there is a compiled version available on the [MultiworldGG releases page](https://github.com/MultiworldGG/MultiworldGG/releases), use that version.
To build the full MultiworldGG software stack, refer to [Running From Source](running%20from%20source.md).
...
Starting from the root repository directory, the standalone MultiworldGG image can be built and run with the command:
`docker build -t multiworldgg .`
Or:
`podman build -t multiworldgg .`
...
`docker run --network host multiworldgg`
Or:
`podman run --network host multiworldgg`

The MultiworldGG web host requires access to multiple ports in order to host game servers simultaneously. To simplify configuration for this purpose, specify `--network host`.
...
`docker run multiworldgg --network host -v /path/to/config.yaml:/app/config.yaml`
See `docs/webhost configuration sample.yaml` for example.


## Using Docker Compose

An example [docker compose](../deploy/docker-compose.yml) file can be found in [deploy](../deploy), along with example configuration files used by the services it orchestrates. Using these files as-is will spin up two separate multiworldgg containers with special modifications to their runtime arguments, in addition to deploying an `nginx` reverse proxy container.
...
`docker run multiworldgg -v "/path/to/zelda.sfc:/app/Zelda no Densetsu - Kamigami no Triforce (Japan).sfc"`.
Enemizer is not currently available for `aarch64`.


## Optional: Git

Building the image requires a local copy of the MultiworldGG source code.
